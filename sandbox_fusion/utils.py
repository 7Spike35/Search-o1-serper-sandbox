...
def check_correctness(
    ...
    # print(type(in_outs))
    # print(in_outs)
    if type(in_outs) is str:
        tmp = json.dumps(in_outs)
        in_outs = json.loads(tmp)
        in_outs = ast.literal_eval(in_outs)
    if type(in_outs["inputs"]) is str:
        inputs = ast.literal_eval(in_outs["inputs"])
    else:
        inputs = in_outs["inputs"]
    if type(in_outs["outputs"]) is str:
        expected_outputs = ast.literal_eval(in_outs["outputs"])
    else:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(32, os.cpu_count() * 5)) as executor:
        ...
        future_to_index = {
            executor.submit(
                ...
                expected_outputs[i],
                sandbox_fusion_url,
                (generation + "\n\n" + str(assert_cases[i])) if assert_cases[i] else generation,  # Append assert case to generation
                timeout,
                memory_limit_mb,
                language,
...
if __name__ == "__main__":
    ...
    print(type(x))
    print(x)    
#     # 测试1: 不使用fn_name（当前失败的方式）
#     in_outs_no_fn = {
#         "inputs": ["test"],
#         "outputs": ["hello"]
#     }
    
#     code_no_fn = "print('hello')"
    
#     print("=== 测试1: 没有fn_name ===")
#     result1, metadata1 = check_correctness(
#         sandbox_fusion_url=sandbox_url,
#         in_outs=in_outs_no_fn,
#         generation=code_no_fn,
#         timeout=30
#     )
#     print('result:', result1)
#     print('metadata1:',metadata1)
#     print('stdout:', metadata1[0].get('stdout') if metadata1 else 'No metadata')
    
#     # 测试2: 使用fn_name
#     in_outs_with_fn = {
#         "inputs": ['"test"'],  # 注意：函数模式需要参数列表
#         "outputs": ["hello"],
...
#     print("\n=== 测试2: 有fn_name ===")
#     result2, metadata2 = check_correctness(
#         sandbox_fusion_url=sandbox_url,
#         in_outs=in_outs_with_fn,
#         generation=code_with_fn,
#         timeout=30
#     )
#     print('result:', result2)
#     print('metadata2:',metadata2)
#     print('stdout:', metadata2[0].get('stdout') if metadata2 else 'No metadata')